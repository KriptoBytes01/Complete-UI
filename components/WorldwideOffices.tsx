  ...
  return (
    <div className="bg-transparent text-black p-10 lg:p-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-8 mb-16">
        <div>
          <h1 className="md:text-5xl text-3xl mb-4 text-black">Worldwide offices</h1>
          <p className="text-black md:text-lg">Explore our service offerings and subsidiaries in specific geographies.</p>
        </div>
        <div className="flex flex-row gap-2 md:px-14 md:py-14 py-4 w-full sm:w-fit bg-transparent text-center">
          <div>
      ...
      {/* Buttons */}
      <div className="flex flex-wrap md:gap-4 gap-2 mb-16 justify-center">
        {buttonData.map((button, index) => (
          <button
            key={index}
            className={`md:px-6 p-2 md:py-3 hover:bg-teal-500 hover:text-white rounded-3xl text-sm border-2 transition duration-300 ${
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
        {filteredBoxes.map((box, index) => (
          <div
            key={index}
            className="border-2 bg-white justify-center border-gray-400 bg-transparent flex flex-col items-center p-6 md:p-10 "
          >
            <h3 className="md:text-3xl text-xl text-center text-black">{box.heading}</h3>
