  ...
  return (
    <>
      ...
      <div className="w-full bg-white border-b border-gray-200">
        <div className="max-w-screen-xl md:px-12 px-2 py-8 mx-auto flex justify-between items-center p-4">
          <h1 className="text-2xl md:text-5xl mt-8 text-black font-semibold">Insights</h1>

          <div className="flex-col md:flex md:items-center items-end space-x-1 md:space-x-6">
            {/* Industry Dropdown */}
            <div className="relative">
              <select
                className="md:px-3 py-2 outline-none"
            ...
            {/* Topic Dropdown */}
            <div className="relative">
              <select className="px-3 py-2 outline-none" aria-label="Select Topic">
      ...
      <div className="max-w-screen-xl mx-auto flex flex-col lg:flex-row py-10 pb-12 space-y-6 lg:space-y-0 lg:space-x-6">
        ...
        <div className="w-full lg:w-2/3 grid grid-cols-1 gap-10">
          {visibleCards.map(({ id, image, heading, date, categories, paragraph }) => (
            <div
              key={id}
              className="bg-white transition-all duration-300 transform hover:scale-105 hover:translate-x-2 hover:translate-y-2 px-10 py-10 relative flex flex-col sm:flex-row"
            >
              ...
              >
                <svg
                  className={`w-6 h-6 ${
                    savedCards[id.toString()] ? "text-white" : "text-gray-600"
                  }`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
        ...
        <div className="w-full px-3 lg:w-1/3 space-y-6">
          {/* Buttons */}
          <div className="grid items-end justify-end grid-cols-2 gap-4">
            {buttonData.map(({ id, label }) => (
              <button
                key={id}
                className="w-fit bg-transparent border-2 border-gray-500 text-gray-500 md:py-2 py-1 md:px-3 px-2 hover:bg-gray-500 hover:text-white transition-colors"
              >
                {label}
              </button>
            ))}
          ...
          {/* Box with Image */}
          <div
            className="relative md:mt-20 mt-16 h-96 bg-cover bg-center"
            style={{ backgroundImage: 'url("/box-image.png")' }}
          >
            <div className="absolute md:py-12 py-8 md:px-8 px-1 inset-0 flex items-center justify-center bg-black bg-opacity-50">
              <div className="text-start text-white px-8 py-6">
                ...
                <p className="mt-2 mb-4">
                  Almost every college and university has moved towards
                  e-learning platforms due to the pandemic. The intro
                </p>
